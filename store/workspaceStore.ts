...
const defaultElementStyle = {
  rectangle: { fill: "#cfe1df", stroke: "#1f6f78", strokeWidth: 2, opacity: 1, fontSize: 16 },
  rect:      { fill: "#cfe1df", stroke: "#1f6f78", strokeWidth: 2, opacity: 1, fontSize: 16 },
  circle:    { fill: "#f0dcc3", stroke: "#b36a21", strokeWidth: 2, opacity: 1, fontSize: 16 },
  text:      { fill: "#1e2523", stroke: "transparent", strokeWidth: 0, opacity: 1, fontSize: 28 }
...
export const useWorkspaceStore = create<WorkspaceState>()(
  persist(
    (set, get) => ({
      ...
      clear: () =>
        set({
          ...
          elements: starterElements, elementList: starterElements,
          selectedElementId: null, loading: false
        }),
      addElement: (type) =>
        set((state) => {
          const nextElement = createElement(type, state.elements.length);
          const nextElements = [...state.elements, nextElement];
          return { ...setElementCollections(nextElements), selectedElementId: nextElement.id };
        }),
      ...
      updateElementStyle: (elementId, style) => {
        const current = get().elements.find((el) => el.id === elementId);
        if (!current) return;
        get().updateElement(elementId, { style: { ...current.style, ...style } });
      },
      updateLayerOrder: (elements) =>
        set(() => {
          const nextElements = normalizeElements(
            elements.map((el, index) => withCompatFields({ ...el, layerOrder: index, layer_order: index }))
          );
          return setElementCollections(nextElements);
        }),
      duplicateSelectedElement: () =>
        set((state) => {
          ...
          return { ...setElementCollections([...state.elements, duplicate]), selectedElementId: duplicate.id };
        }),
        ...
        set((state) => {
          ...
          const remaining = normalizeElements(
            state.elements
              ...
              .map((el, index) => withCompatFields({ ...el, layerOrder: index, layer_order: index }))
          );
          return { ...setElementCollections(remaining), selectedElementId: remaining.at(-1)?.id ?? null };
        }),
      ...
      reorderElement: (elementId, direction) =>
        set((state) => {
          ...
          return setElementCollections(
            ordered.map((el, index) => withCompatFields({ ...el, layerOrder: index, layer_order: index }))
          );
        }),
      saveSnapshot: () =>
        set((state) => {
          ...
          return { snapshots: [snapshot, ...state.snapshots].slice(0, 10) };
        }),
    ...
    }),
    {
      ...
      partialize: (state) => ({ elements: state.elements, workspaceName: state.workspaceName }),
...
export function getWorkspaceExport() {
  ...
  return JSON.stringify(
    { workspace: state.workspace, workspaceName: state.workspaceName,
      selectedElementId: state.selectedElementId, elements: state.elements },
    null, 2
  );
}
